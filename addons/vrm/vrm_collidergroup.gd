...
func _ready(parent: Node3D, skel: Skeleton3D):
	self.parent = parent
	self.skel = skel
...
class SphereCollider:
	...
	func update(parent: Node3D, skel: Skeleton3D):
		if idx != -1:
			position = VRMTopLevel.VRMUtil.transform_point((skel.global_transform * skel.get_bone_rest(idx).affine_inverse() * skel.get_bone_global_pose_without_override(idx)), offset)
		else:
