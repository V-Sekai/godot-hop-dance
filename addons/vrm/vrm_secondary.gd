...
class SecondaryGizmo:
	...
	func draw_in_editor(do_draw_spring_bones: bool = false):
		...
		_commit_arraymesh() # Remove when we can use the real ImmediateGeomtry3D
	...
	func draw_spring_bones(color: Color):
		...
		for spring_bone in secondary_node.spring_bones_internal:
			for v in spring_bone.verlets:
				...
				s_tr = s_sk.get_bone_rest(v.bone_idx).affine_inverse() * s_sk.get_bone_global_pose(v.bone_idx)
				draw_line(
					s_tr.origin,
					VRMTopLevel.VRMUtil.inv_transform_point(s_sk.get_bone_rest(v.bone_idx) * s_sk.global_transform, v.current_tail),
					color
				)
				draw_sphere(
					s_tr.basis,
		...
		for collider_group in (secondary_node.collider_groups if Engine.is_editor_hint() else secondary_node.collider_groups_internal):
			...
			if Engine.is_editor_hint():
				...
				if c_sk is Skeleton3D:
					c_tr = c_sk.get_bone_rest(c_sk.find_bone(collider_group.bone)).affine_inverse() * c_sk.get_bone_global_pose(c_sk.find_bone(collider_group.bone))
			elif collider_group.parent is Skeleton3D:
				c_tr = collider_group.skel.get_bone_rest(collider_group.parent.find_bone(collider_group.bone)).affine_inverse() * collider_group.skel.get_bone_global_pose_without_override(collider_group.parent.find_bone(collider_group.bone))
