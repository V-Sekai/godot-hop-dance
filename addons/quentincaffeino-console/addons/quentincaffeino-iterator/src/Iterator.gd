...
func first() -> Variant: # Variant|null
	if self.length:
...
func last() -> Variant: # Variant|null
	if self.length:
		self._iterationCurrentIndex = self._length - 1
		return self._get(self._iterationCurrentIndex)
...
func next() -> Variant: # Variant|null
	if self.length and self._iterationCurrentIndex < self._length - 1:
...
func previous() -> Variant: # Variant|null
	if self._length > 0 and self._iterationCurrentIndex > 0:
		self._iterationCurrentIndex -= 1
		return self._get(self._iterationCurrentIndex)
...
func current() -> Variant: # Variant|null
	if self.length:
		return self._get(self._iterationCurrentIndex)
...
func _iter_init(arg: Variant) -> bool:
	self._iterationCurrentIndex = 0
	return self._iterationCurrentIndex < self._length
